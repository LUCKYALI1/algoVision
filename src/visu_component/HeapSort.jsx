...
// --- Theory Card for Heap Sort ---
const HeapSortTheoryCard = () => (
    <div className="mt-8 bg-white dark:bg-slate-900 p-6 rounded-xl shadow-md border border-slate-200 dark:border-slate-800">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">Understanding Heap Sort</h2>
        <div className="prose prose-slate dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
            <p>Heap Sort is a comparison-based sorting technique based on a Binary Heap data structure. It is similar to selection sort where we first find the maximum element and place it at the end.</p>
            <h3 className="text-xl font-semibold mt-6 mb-4">How it Works:</h3>
            <ol>
                <li><strong>Build Max Heap:</strong> Rearrange the array into a max heap, where for any given node, its value is greater than or equal to the values of its children.</li>
                <li><strong>Sort:</strong> The largest item is stored at the root of the heap. Swap it with the last item of the heap, then reduce the heap size by one. Finally, "heapify" the root of the tree to maintain the heap property. Repeat this process until the heap size is one.</li>
            </ol>
            ...
            <div className="my-4 overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                        ...
                        <tr><td className="px-4 py-3 font-mono">Space</td><td className="px-4 py-3 font-mono">O(1)</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
// --- Step Generation Logic for Heap Sort ---
const generateHeapSortSteps = (arr) => {
    ...
    function heapify(size, i) {
        ...
        steps.push({ arrayState: [...localArr], activeIndices: { root: i, comparing: [l, r].filter(idx => idx < size), sorted: [...sortedIndices] }, message: `Heapifying at root ${localArr[i]}.` });
        
        if (l < size && localArr[l] > localArr[largest]) largest = l;
        if (r < size && localArr[r] > localArr[largest]) largest = r;

        if (largest !== i) {
            steps.push({ arrayState: [...localArr], activeIndices: { root: i, swapped: [i, largest], sorted: [...sortedIndices] }, message: `Swapping ${localArr[i]} and ${localArr[largest]}.` });
            [localArr[i], localArr[largest]] = [localArr[largest], localArr[i]];
            steps.push({ arrayState: [...localArr], activeIndices: { root: i, swapped: [i, largest], sorted: [...sortedIndices] }, message: `Swap complete. Recursively heapifying.` });
            heapify(size, largest);
        }
    }

    steps.push({ arrayState: [...localArr], activeIndices: { sorted: [] }, message: "Building max heap..." });
    for (let i = Math.floor(n / 2) - 1; i >= 0; i--) {
        heapify(n, i);
    }
    steps.push({ arrayState: [...localArr], activeIndices: { sorted: [] }, message: "Max heap built. Starting sort phase." });

    for (let i = n - 1; i > 0; i--) {
        steps.push({ arrayState: [...localArr], activeIndices: { swapped: [0, i], sorted: [...sortedIndices] }, message: `Swapping max element ${localArr[0]} with last element ${localArr[i]}.` });
        [localArr[0], localArr[i]] = [localArr[i], localArr[0]];
        sortedIndices.unshift(i);
        steps.push({ arrayState: [...localArr], activeIndices: { sorted: [...sortedIndices] }, message: `Element ${localArr[i]} is now sorted. Heapifying the reduced heap.` });
        heapify(i, 0);
    }
    
    sortedIndices.unshift(0);
    steps.push({ arrayState: [...localArr], activeIndices: { sorted: sortedIndices }, message: "Sorting complete!" });
    return steps;
};

// --- Bar Styling Logic for Heap Sort ---
const getHeapSortBarClass = (index, activeIndices) => {
    ...
    if (index === activeIndices.root) return `${base} bg-sky-500 text-white scale-110`;
...
const HeapSort = () => {
    ...
    const [userInputArray, setUserInputArray] = useState('4, 10, 3, 5, 1');
    ...
    return (
        <div className="bg-slate-100 dark:bg-slate-950 min-h-screen p-4 sm:p-8 font-sans">
            <header className="text-center mb-8">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-800 dark:text-white">Heap Sort Visualizer</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2">A comparison-based sort using a Binary Heap.</p>
            </header>
                    ...
                    <div className="flex justify-center items-center mb-4 h-24">
                        {array.map((value, idx) => (
                            <div key={idx} className="flex flex-col items-center flex-1">
                                <div className={getHeapSortBarClass(idx, activeIndices)}>{value}</div>
