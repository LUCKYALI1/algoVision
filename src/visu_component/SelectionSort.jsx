...
// --- Theory Component for Selection Sort ---
const TheoryCard = () => (
    <div className="mt-8 bg-white dark:bg-slate-900 p-6 rounded-xl shadow-md border border-slate-200 dark:border-slate-800">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">Understanding Selection Sort</h2>
        <div className="prose prose-slate dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
            <p>
                Selection Sort is an in-place comparison sorting algorithm. It divides the input list into two parts: a sorted sublist which is built up from left to right, and a sublist of the remaining unsorted items that occupy the rest of the list.
            </p>
            ...
            <ol>
                ...
                <li>Move the boundary of the sorted and unsorted subarrays one element to the right.</li>
            ...
            <p>Selection Sort's performance is not affected by the initial order of the elements, making its time complexity consistent across all cases.</p>
...
// --- Visualizer Component for Selection Sort ---
const SelectionSort = () => {
    // --- State Management ---
    ...
    const [activeIndices, setActiveIndices] = useState({ currentIndex: null, minIndex: null, comparingIndex: null, sorted: [] });
    
    // User input state
    const [userInputArray, setUserInputArray] = useState('64, 25, 12, 22, 11');
    ...
    const resetState = (clearMessage = false) => {
        ...
        setSteps([]);
        setCurrentStep(0);
        setActiveIndices({ currentIndex: null, minIndex: null, comparingIndex: null, sorted: [] });
        setStatus('idle');
        setInputError('');
        if (clearMessage) {
    ...
    const generateSteps = (arr) => {
        ...
        for (let i = 0; i < n - 1; i++) {
            ...
            newSteps.push({
                arrayState: [...localArr],
                currentIndex: i, minIndex: minIndex, comparingIndex: null, sorted: [...sortedIndices],
                message: `Pass ${i + 1}: Finding the minimum element.`
            });

            for (let j = i + 1; j < n; j++) {
                newSteps.push({
                    arrayState: [...localArr],
                    currentIndex: i, minIndex: minIndex, comparingIndex: j, sorted: [...sortedIndices],
                    message: `Comparing ${localArr[j]} with current minimum ${localArr[minIndex]}.`
                });
                ...
                if (localArr[j] < localArr[minIndex]) {
                    ...
                    newSteps.push({
                        arrayState: [...localArr],
                        currentIndex: i, minIndex: minIndex, comparingIndex: j, sorted: [...sortedIndices],
                        message: `Found new minimum: ${localArr[minIndex]}.`
                    });
                }
            }
            ...
            if (minIndex !== i) {
                newSteps.push({
                    arrayState: [...localArr],
                    currentIndex: i, minIndex: minIndex, comparingIndex: null, sorted: [...sortedIndices],
                    message: `Swapping minimum ${localArr[minIndex]} with element at index ${i}.`
                });
            ...
            newSteps.push({
                arrayState: [...localArr],
                currentIndex: null, minIndex: null, comparingIndex: null, sorted: [...sortedIndices],
                message: `Element ${localArr[i]} is now sorted.`
            });
        }
        ...
        newSteps.push({
            arrayState: [...localArr],
            currentIndex: null, minIndex: null, comparingIndex: null, sorted: allSorted,
    ...
    const getBarClass = (index) => {
        ...
        if (index === activeIndices.minIndex) return `${base} bg-sky-500 text-white shadow-lg scale-110`;
        if (index === activeIndices.currentIndex) return `${base} bg-indigo-500 text-white shadow-md`;
        if (index === activeIndices.comparingIndex) return `${base} bg-yellow-400 dark:bg-yellow-500 text-black shadow-md`;
    ...
    return (
        <div className="bg-slate-100 dark:bg-slate-950 min-h-screen p-4 sm:p-8 font-sans">
            <header className="text-center mb-8">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-800 dark:text-white">Selection Sort Visualizer</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2">An in-place comparison sorting algorithm.</p>
            </header>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl shadow-md border border-slate-200 dark:border-slate-800 mb-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start">
                        <div className="flex-grow w-full">
                            <label htmlFor="array-input-selection" className="block text-sm font-medium text-black dark:text-white mb-1">Array (comma-separated)</label>
                            <input id="array-input-selection" type="text" value={userInputArray} onChange={e => setUserInputArray(e.target.value)} placeholder="e.g., 64, 25, 12, 22, 11" className="text-black dark:text-white w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg p-2 font-mono"/>
                        </div>
